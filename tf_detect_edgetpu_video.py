...
def draw_image(image, results, labels, size):
    ...
    for idx, obj in enumerate(results):
        ...
        cv2.putText(image, display_str,(xmin,ymin),0, 0.75, (255,255,255),1)
...
def detect_objects(interpreter, image, threshold):
    ...
    inp_id = interpreter.get_input_details()[0]["index"]
    interpreter.set_tensor(inp_id, image)
...
def main():
    ...
    while(video.isOpened()):
        try:
            ...
            t2=time.time()
            print(t2-t1)
            draw_image(frame, results, labels, frame.shape)
